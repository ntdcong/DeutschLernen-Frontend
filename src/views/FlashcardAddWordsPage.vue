      ...
      <div class="mx-auto max-w-6xl">
        ...
        <div class="grid gap-8 lg:grid-cols-3">
          ...
          <div class="lg:col-span-1 space-y-6">
            <!-- Tabs -->
            <div class="flex rounded-lg bg-surface-light p-1 dark:bg-surface-dark border border-border-light dark:border-border-dark">
              <button
                v-for="tab in tabs"
                :key="tab.id"
            ...
            <!-- Manual Input -->
            <div v-if="currentTab === 'manual'" class="rounded-xl border border-border-light bg-surface-light p-6 dark:border-border-dark dark:bg-surface-dark">
              ...
              <form @submit.prevent="addManualWord" class="space-y-4">
                <div>
                  <label class="mb-1 block text-sm font-medium text-text-secondary-light dark:text-text-secondary-dark">Từ vựng (Word)</label>
                  <input
                    v-model="manualForm.word"
                    required
                ...
                <div>
                  <label class="mb-1 block text-sm font-medium text-text-secondary-light dark:text-text-secondary-dark">Nghĩa (Meaning)</label>
                  <input
                    v-model="manualForm.meaning"
                    required
                    class="w-full rounded-lg border border-border-light bg-background-light px-4 py-2 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/50 dark:border-border-dark dark:bg-background-dark"
                ...
                <div>
                  <label class="mb-1 block text-sm font-medium text-text-secondary-light dark:text-text-secondary-dark">Giống (Genus)</label>
                  <select
                    v-model="manualForm.genus"
                    class="w-full rounded-lg border border-border-light bg-background-light px-4 py-2 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/50 dark:border-border-dark dark:bg-background-dark"
                  >
            ...
            <!-- Excel Input -->
            <div v-if="currentTab === 'excel'" class="rounded-xl border border-border-light bg-surface-light p-6 dark:border-border-dark dark:bg-surface-dark">
              ...
              >
                ...
                />
                <span class="material-symbols-outlined mb-2 text-4xl text-text-secondary-light dark:text-text-secondary-dark">upload_file</span>
            ...
            <!-- AI Input -->
            <div v-if="currentTab === 'ai'" class="rounded-xl border border-border-light bg-surface-light p-6 dark:border-border-dark dark:bg-surface-dark">
              <h3 class="mb-4 text-lg font-bold text-primary-black dark:text-white">Tạo bằng AI</h3>
              <div class="space-y-4">
                <div>
                  <label class="mb-1 block text-sm font-medium text-text-secondary-light dark:text-text-secondary-dark">Chủ đề (Topic)</label>
                  <input
                ...
                <div>
                  <label class="mb-1 block text-sm font-medium text-text-secondary-light dark:text-text-secondary-dark">Trình độ</label>
                  <select
                ...
                <div>
                  <label class="mb-1 block text-sm font-medium text-text-secondary-light dark:text-text-secondary-dark">Số lượng từ</label>
                  <input
                ...
                <p class="text-xs text-text-secondary-light dark:text-text-secondary-dark">Lưu ý: Tạo từ vựng bằng AI thì sẽ thêm thẳng vào Bộ từ vựng và sẽ không preview được!</p>
                <button
                  @click="generateWords"
          ...
          <div class="lg:col-span-2 flex flex-col h-[calc(100vh-12rem)]">
            <div class="mb-4 flex items-center justify-between">
              ...
              <div class="flex gap-2">
                ...
                <button
                  @click="saveAll"
                  :disabled="pendingWords.length === 0 || isSaving"
            ...
            <div class="flex-1 overflow-hidden rounded-xl border border-border-light bg-surface-light dark:border-border-dark dark:bg-surface-dark flex flex-col">
              <div class="overflow-y-auto flex-1 p-0">
                <table class="w-full text-left text-sm">
                  <thead class="sticky top-0 bg-gray-50 text-xs uppercase text-text-secondary-light dark:bg-gray-800 dark:text-text-secondary-dark">
                    <tr>
                  ...
                  <tbody class="divide-y divide-border-light dark:divide-border-dark">
                    ...
                    <tr v-for="(word, index) in paginatedWords" :key="index" class="group hover:bg-gray-50 dark:hover:bg-white/5">
                      <td class="px-6 py-4">
                        <input
                          v-model="word.word"
                      ...
                      <td class="px-6 py-4 text-right">
                        <button
                          @click="removeWord(index + (currentPage - 1) * itemsPerPage)"
